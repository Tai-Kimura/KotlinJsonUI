...
#   curl -fsSL https://raw.githubusercontent.com/Tai-Kimura/KotlinJsonUI/main/installer/bootstrap.sh | bash -s -- -v 1.0.0
#   curl -fsSL https://raw.githubusercontent.com/Tai-Kimura/KotlinJsonUI/main/installer/bootstrap.sh | bash -s -- -d ./my-project
#   curl -fsSL https://raw.githubusercontent.com/Tai-Kimura/KotlinJsonUI/main/installer/bootstrap.sh | bash -s -- -v 1.0.0 -s

set -e

# Configuration
GITHUB_REPO="Tai-Kimura/KotlinJsonUI"
INSTALLER_PATH="installer/install_kjui.sh"
...
check_node_environment() {
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null; then
        ...
        NPM_VERSION=$(npm -v | grep -oE '[0-9]+' | head -1)
        print_info "Found Node.js v$NODE_VERSION and npm v$NPM_VERSION"
        
        # Check minimum Node.js version (14+)
        if [ "$NODE_VERSION" -ge 14 ]; then
            print_info "Node.js version is compatible"
            return 0
        else
            print_warning "Node.js version is older than recommended (14+)"
            print_info "Consider updating Node.js for better compatibility"
        fi
    else
        print_warning "Node.js/npm not found!"
        print_info "Node.js is required for HotLoader functionality"
...
print_info "KotlinJsonUI Bootstrap"
...
# Check Node.js environment (optional but recommended)
check_node_environment
...
print_info "1. Navigate to your Android project directory"
print_info "2. Run: kjui_tools/bin/kjui init"
print_info "3. Run: kjui_tools/bin/kjui help"