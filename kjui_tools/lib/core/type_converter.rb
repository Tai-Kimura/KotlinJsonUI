  ...
  module Core
    ...
    class TypeConverter
      ...
      class << self
        ...
        # @param type_str [String] the function type string, e.g., "((Boolean) -> Unit)?"
        # @return [Array<String>] array of parameter type strings (raw, not converted)
        # @example
        #   extract_function_parameter_types("((Boolean) -> Unit)?") # => ["Boolean"]
        #   extract_function_parameter_types("((Int, String) -> Unit)?") # => ["Int", "String"]
        #   extract_function_parameter_types("(() -> Unit)?") # => []
        ...
        # @example
        #   event_handler_mode("((Boolean) -> Unit)?") # => :value
        #   event_handler_mode("((Event) -> Unit)?") # => :event
        #   event_handler_mode("(() -> Unit)?") # => :none
