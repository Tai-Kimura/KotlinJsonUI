  ...
  module Core
    module Resources
      class ColorManager
        ...
        def process_colors(processed_files, processed_count, skipped_count)
          ...
          # Disabled: ColorManager.kt generation is not needed
        ...
        def generate_kotlin_code(colors)
          ...
          code << "    )"
          code << ""
          code << "    // Android View colors"
          code << "    object android {"
          code << "        // Get Color by key"
          code << "        fun color(key: String): Int {"
          code << "            val hexString = colorsData[key]"
          code << "            if (hexString == null) {"
          code << "                println(\"Warning: Color key '$key' not found in colors.json\")"
          code << "                return Color.GRAY // Default fallback color"
          code << "            }"
          code << "            return try {"
          code << "                Color.parseColor(hexString)"
          code << "            } catch (e: IllegalArgumentException) {"
          code << "                println(\"Warning: Invalid color format '$hexString' for key '$key'\")"
          code << "                Color.GRAY"
          code << "            }"
          code << "        }"
          code << ""
          
          # Generate static color accessors for Android
          colors.keys.sort.each do |key|
            ...
            if @colors_data[key]
              ...
              code << "                println(\"Warning: Color '#{key}' is not defined in colors.json\")"
          ...
          code << "    // Compose colors"
          code << "    object compose {"
          code << "        // Get Compose Color by key"
          code << "        fun color(key: String): ComposeColor {"
          code << "            val androidColor = android.color(key)"
          ...
          colors.keys.sort.each do |key|
            ...
            code << "            get() = ComposeColor(android.#{property_name})"
            code << ""
          end
